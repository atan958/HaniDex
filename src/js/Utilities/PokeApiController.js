...
/*
/ Below are variations of the above methods => Used to compare the speed of the calls
/
/ Note: *** TO BE REMOVED ***
*/
const retrievePokeApiData1 = async () => {
    console.log('STARTING ' + new Date().getTime());
    let filteringTasks = []
    let pokeApiData_pkmSpc = (await axios.get(pokeApiUrl_pkmSpc)).data;
    filteringTasks = filteringTasks.concat((getFilteredData(pokeApiData_pkmSpc)));
    ...
    let filteredDataRes = await Promise.all(filteringTasks);
    console.log('FINISHED ' + new Date().getTime());
    let filteredData = [];
    filteredDataRes.forEach((res) => {
        filteredData = filteredData.concat(res);
    });
...
/ Each object contains the respective pokemon's name, number, description, stats, etc.
*/
const getFilteredData1 = async (pokeApiData_pkmSpc) => {
    /*
    / Acquires necessary data for each and all Pokemon in the list concurrently
    */
    const getTheData = async (pkmSpc) => {
    ...
    const getDataRes = pokeApiData_pkmSpc.results.map((pkmSpc) => {
        return getTheData(pkmSpc);
    });
    const filteredData = await Promise.all(getDataRes);
    

    //const filteredData = [await getTheData(pokeApiData_pkmSpc.results[0])];
...
/ Note: Total number of retrievable Pokemon species is 898
*/
const retrievePokeApiData2 = async () => {
    console.log('STARTING ' + new Date().getTime());
    let asyncTasks = []
    let pokeApiData_pkmSpc = (await axios.get(pokeApiUrl_pkmSpc)).data;
    asyncTasks.push(getFilteredData(pokeApiData_pkmSpc));
    ...
    console.log('FINISHED ' + new Date().getTime());
    let filteredDataRes = await Promise.all(asyncTasks);
    let filteredData = [];
...
/ Each object contains the respective pokemon's name, number, description, stats, etc.
*/
const getFilteredData2 = async (pokeApiData_pkmSpc) => {
    /*
    / Acquires necessary data for each and all Pokemon in the list concurrently
    */
    const filterDataAsync = async (pkmSpc) => {
    ...
    let filteredData = await Promise.all(pokeApiData_pkmSpc.results.map((pkmSpc) => {
        return filterDataAsync(pkmSpc);
    }));
